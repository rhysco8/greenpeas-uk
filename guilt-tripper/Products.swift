//
//  Products.swift
//  guilt-tripper
//
//  Created by Student on 14/08/2019.
//  Copyright © 2019 Greenpeas UK. All rights reserved.
//

import Foundation


let palmOilProducts = ["Nutella", "Pot Noodle Chicken and Mushroom flavour", "Kelloggs Crunchy Nut Clusters", "Warburtons Toastie Soft White Bread", "Vitalite Dairy Free Butter", "Cardamom & Honey Granola", "Cadbury Dairy Milk Whole Nut Chocolate Bar"]
let nonPalmOilProducts = ["Biona Organic Dark Chocolate Spread", "Biona Organic Rye Bread", "Kallo Milk Chocolate Rice Cakes", "Kallo Organic Wholegrain Puffed Rice Cereal", "Pip & Nut Smooth Peanut Butter", "Waitrose Dark Chocolate Spread" ]
